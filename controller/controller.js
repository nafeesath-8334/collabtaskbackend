...
exports.editProfile = async (req, res) => {
  try {
    const userId = req.params.id; // expect route: /users/:id
    console.log("userId", userId)
    let imageUrl = null;
    if (req.file && req.file.filename) {
      imageUrl = `/profileImages/${req.file.filename}`;
    }
...
exports.createTeam = async (req, res) => {
  try {
    const { teamName, teamDescription, teamMembers, projectId } = req.body;
    const ownerId = req.user.id; // from jwtMiddleware
    ...
    // Add team reference to each user's profile
    await User.updateMany(
      { _id: { $in: [ownerId, ...teamMembers] } },
      { $addToSet: { teams: savedTeam._id } }
    );

    // 👉 Also add the team to the project
    if (projectId) {
      await Project.findByIdAndUpdate(
        projectId,
        { team: savedTeam._id }, // overwrite instead of pushing to array
        { new: true }
      );
    }
